...
interface AdminCollegesPaginationProps {
  pagination: PaginationType;
  onPageChange: (page: number) => void;
  onLimitChange: (limit: number) => void;
}

export function AdminCollegesPagination({
  pagination,
  onPageChange,
  onLimitChange,
}: AdminCollegesPaginationProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2 text-xs font-medium text-slate-600">
      {/* Left Text: Showing 1-10 of 186 colleges */}
      <div className="text-slate-500 font-semibold">
        Showing <span className="text-slate-900 font-bold">{startIdx}-{endIdx}</span> of{' '}
        <span className="text-slate-900 font-bold">{total.toLocaleString()}</span> colleges
      </div>
